...
class MixinDataclassArray:
    ...
    def __array__(self, dtype: Optional[DType] = None) -> NDArray:
        """
    ...
    def arithmetical_operation(self,
                               a: Any,
                               operation: str,
                               in_place: Boolean = False) -> Dataclass:
        """
...
def as_array(a: Union[ArrayLike, List[FloatingOrNDArray]],
             dtype: Optional[DType] = None) -> NDArray:
    """
...
def as_int(a: IntegerOrArrayLike,
           dtype: Optional[DTypeInteger] = None) -> IntegerOrNDArray:
    """
    ...
    return dtype(np.squeeze(a))  # type: ignore[operator]


def as_float(a: FloatingOrArrayLike,
             dtype: Optional[DTypeFloating] = None) -> FloatingOrNDArray:
    """
    ...
    return dtype(a)  # type: ignore[operator]


def as_int_array(a: ArrayLike,
                 dtype: Optional[DTypeInteger] = None) -> NDArray:
    """
    Converts given variable :math:`a` to :class:`numpy.ndarray` using given
    :class:`numpy.dtype`.
...
def as_float_array(a: ArrayLike,
                   dtype: Optional[DTypeFloating] = None) -> NDArray:
    """
    Converts given variable :math:`a` to :class:`numpy.ndarray` using given
    :class:`numpy.dtype`.
...
def as_int_scalar(a: IntegerOrArrayLike,
                  dtype: Optional[DTypeInteger] = None) -> Integer:
    """
    ...
    return as_int(a, dtype)  # type: ignore[return-value]


def as_float_scalar(a: FloatingOrArrayLike,
                    dtype: Optional[DTypeFloating] = None) -> Floating:
    """
    ...
    attest(a.ndim == 0,
           '"{0}" cannot be converted to "float" scalar!'.format(a))

    return as_float(a, dtype)  # type: ignore[return-value]


def set_float_precision(dtype: DType = DEFAULT_FLOAT_DTYPE) -> None:
    """
...
def set_int_precision(dtype: DType = DEFAULT_INT_DTYPE) -> None:
    """
...
) -> Union[Literal['ignore', 'reference', '1', '100'], str]:
    """
...
def set_domain_range_scale(scale: Union[Literal['ignore', 'reference'
                                                'Ignore', 'Reference', '1',
                                                '100'], str] = 'reference'):
    """
    ...
    _DOMAIN_RANGE_SCALE = validate_method(
        str(scale), ('ignore', 'reference', '1', '100'),
...
class domain_range_scale:
    ...
    def __init__(
            self,
            scale: Union[Literal['ignore', 'reference'
                                 'Ignore', 'Reference', '1', '100'], str]):
        self._scale = scale
...
def to_domain_degrees(a: ArrayLike,
...
def from_range_100(a: ArrayLike,
                   scale_factor: FloatingOrArrayLike = 100,
                   dtype: Optional[DTypeFloating] = None) -> NDArray:
    """
...
def from_range_degrees(a: ArrayLike,
                       scale_factor: FloatingOrArrayLike = 360,
                       dtype: Optional[DTypeFloating] = None) -> NDArray:
    """
...
def from_range_int(a: ArrayLike,
                   bit_depth: IntegerOrArrayLike = 8,
                   dtype: Optional[DTypeFloating] = None) -> NDArray:
    """
...
def closest_indexes(a: ArrayLike, b: ArrayLike) -> NDArray:
    ...
    return np.abs(a - b).argmin(axis=0)  # type: ignore[operator]
...
def in_array(a: ArrayLike, b: ArrayLike,
             tolerance: Number = EPSILON) -> NDArray:
    """
...
def tstack(a: Union[ArrayLike, List[FloatingOrNDArray]],
           dtype: Optional[DType] = None) -> NDArray:
    """
...
def tsplit(a: Union[ArrayLike, List[FloatingOrNDArray]],
           dtype: Optional[DType] = None) -> NDArray:
    """
...
def orient(a: ArrayLike,
           orientation: Union[Literal['Flip', 'Flop', '90 CW', '90 CCW',
                                      '180'], str]) -> Union[NDArray, None]:
    """
...
def fill_nan(a: ArrayLike,
             method: Union[Literal['Interpolation', 'Constant'],
                           str] = 'Interpolation',
             default: Number = 0) -> NDArray:
    """
...
def zeros(shape: ArrayLike,
          dtype: Optional[DType] = None,
          order: Literal['C', 'F'] = 'C') -> NDArray:
    """
    ...
    return np.zeros(shape, dtype, order)  # type: ignore[arg-type]


def ones(shape: ArrayLike,
         dtype: Optional[DType] = None,
         order: Literal['C', 'F'] = 'C') -> NDArray:
    """
    ...
    return np.ones(shape, dtype, order)  # type: ignore[arg-type]


def full(shape: ArrayLike,
         fill_value: Number,
         dtype: Optional[DType] = None,
         order: Literal['C', 'F'] = 'C') -> NDArray:
    """
    ...
    return np.full(shape, fill_value, dtype, order)  # type: ignore[arg-type]
